...
- ✅ Library-owned subcommand combinator
  - `ArgParse/Core/Combinators.subcommand` now centralises token dispatch with `Alternative`; `Spec/Elab` and `Main.lean` use it so examples remain applicative-only.
...
- 2025-09-30: Extended `runtimeLinesForSummary` and `runtimeParagraphs` to handle merged option/positional summaries via the new merge lemmas.
...
- 2025-09-29: Reworked `Main.lean` to mirror the legacy applicative combinator example: the CLI now builds a typed `Parser AppCommand` (with subcommands) while sharing spec metadata for docs/completions, and dispatches via `ArgParse.builtinOutcome?`.
- 2025-09-29: Rebuilt `Main.lean` with the greeting demo using `AppSpec`, `runSummary`, and summary queries; CLI now prints greetings, handles `--help`/`--man`/`--generate-completions`, and formats parser errors via `renderError`.
